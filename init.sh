...
function reset()
{
  docker-machine rm centos7-vm1 -y
  vagrant destroy -f
}
...
function main()
{
  ...
  # Using ~ as delimiter
  sed -i '' -e "s~centos7.vm.synced_folder.*$~${vagrant_synced_folder}~g" Vagrantfile
  ...
  #  echo "ERROR: Current docker-machine version = ${docker_machine_version}"
    ...
    --generic-ssh-key=.vagrant/machines/centos7/virtualbox/private_key \
